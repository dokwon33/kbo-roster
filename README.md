...
| 데이터 | 캐시 키 | 유지시간 | 이유 |
|---|---|---|---|
| 좌석 매진율 통계 | `attendance_rows` | 24시간 | 하루 1회 정도만 바뀌면 충분 |
| 리그 순위(1군/2군) | `standings_1gun_{오늘 날짜}` 등 | 24시간(날짜별 키) | 캐시 키에 날짜를 넣어, sync_roster(크론)가 실패하거나 경기가 없는 날이어도 날짜가 바뀌면 그날 첫 방문 때 자동으로 다시 스크래핑한다. 같은 날 안에서는 재사용해 매 방문마다 긁지 않는다 |
